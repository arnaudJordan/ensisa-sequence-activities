...
public class MDIDesktopPane extends JDesktopPane {
    ...
    public Component add(JInternalFrame frame) {
        ...
        if (frame.isResizable()) {
        	Dimension d=getVisibleRect().getSize();
            if (getBorder() != null) {
    ...
    public void cascadeFrames() {
        ...
        int frameHeight = (d.height - 5) - frames.size() * FRAME_OFFSET;
        int frameWidth = (d.width - 5) - frames.size() * FRAME_OFFSET;
    ...
    public void tileFrames() {
        ...
        if (totalHeight > d.height 
        	...
        	frameWidth -= manager.getScrollPane().getVerticalScrollBar().getPreferredSize().width;
        //int frameHeight = getBounds().height/allFrames.length;
        int frameHeight = 0;
        int y = 0;
        /*for (int j = 0; j < allFrames.length; j++) {
        	frameHeight = allFrames[j].getHeight();
            allFrames[j].setSize(frameWidth,frameHeight);
            allFrames[j].setLocation(0,y);
            y = y + frameHeight;
        }*/
        for(JInternalFrame f : frames) {
